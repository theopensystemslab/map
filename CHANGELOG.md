...
- feat: `drawType` prop adds supported value `"Circle"` for drawing & modifying circles; please note this type is still quite experimental and does _not_ yet dispatch a `geojsonChange` event ("circles" are not natively supported by the geojson format and we'll need to transform to polygons first) ([#465](https://github.com/theopensystemslab/map/pull/465))
